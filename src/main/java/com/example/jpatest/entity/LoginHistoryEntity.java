...
/*
    ...
    연관 관계 정의 규칙
    1. 방향 설정 - 단방향, 양방향
    2. 연관 관계의 주인 : 양방향일때는 외래키를 가지는 테이블이 주인이다.
                        양방향일때는 mapper로 설정을 해줘야한다.
                        연관관계의 주인은 조회, 수정, 저장, 삭제를 할 수 있다.
                        주인이 아니면 조회만 가능하다.
    3. 다중성 : 일대일, 다대일, 일대다, 다대다

        다대일 - 게시글에 여러개의 댓글 작성 기능
        ...
        일대 일
            외래키를 주 테이블에 포함한다.
                ...
                양방향
                    이미지 테이즐에 mappedBy를 설정한다.
        일대다
            주 테이블이 일이 되고 대상 테이블이 다가 되는 관계
            회원테이블이 주 테이블이고, 이미지 테이블이 대상 테이블이라면
            회원 테이블에 이미지테이블의 기본키를 외래키로 설정해야 한다.
            데이터베이스에서는 주테이블을 다 쪽으로 본다.
            그래서 회원객체로 이미지 객체를 저장하게 되면 insert 한번 동작하고, update가 또 한번 동작한다.
        
        다대다
            쇼핑몰에서 상품을 장바구니 담아두는 기능을 구현한다.
            장바구니 테이블과 상품 테이블은 다대일 관계를 가지지만
            상품의 수량이 여러개인 상태에서 다대다로 관계를 가지게 되면
            조인 쿼리문이 복잡해진다. 숨겨져있는 뷰 테이블이 존재하게 될 수도 있다.
            다대다를 해야 되는 경우는 일대다, 다대일로 풀어서 만드는 것이 좋다.

        Optional은 자바의 내장 클래스이다.
        데이터를 다루는 객체를 생성하거나 할 때 사용되는 클래스
        데이터를 읽어서 객체로 생성할때 데이터가 없다면 객체는 생성되지않는다.
        이때 optional을 이용하면 코드의 안정성을 확보할 수 있다.
        데이터를 읽어서 option으로 저장하고 optional에 null이 있는지 여부를 확인하여 동작시킨다.
